        ...
        <View style={{ alignSelf: 'stretch', marginHorizontal: 16, gap: 4, flexGrow: 1, paddingTop: safeArea.top, paddingBottom: safeArea.bottom }}>
            ...
            {layout === 'large' && (
                <View style={{ height: 64, alignItems: 'center', flexDirection: 'row', marginLeft: 8, marginTop: 2 }}>
                    <Image source={require('@/app/assets/home-splash.png')} style={{ width: 24, height: 24 }} />
                    <Text style={{ color: Theme.text, marginLeft: 12, fontSize: 24 }}>GitChats</Text>
                </View>
            )}
    ...
    return (
        <Stack.Screen
            options={{
                ...
                // title: layout === 'small' ? 'GitChats' : 'Home',
...
const HeaderView = React.memo(() => {
    ...
    return (
        <View style={{ flexDirection: 'column', alignItems: Platform.OS === 'ios' ? 'center' : 'flex-start' }}>
            <Text style={{ color: Theme.text, fontSize: 20, fontWeight: '600' }}>GitChats</Text>
            {/* {subtitle && (
