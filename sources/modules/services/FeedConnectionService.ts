...
export class FeedConnectionService {
    ...
    constructor(id: string, users: UserService, client: BackendClient, jotai: Jotai) {
        this.id = id;
        this.client = client;
        this.jotai = jotai;
        this.users = users;
    ...
    #doSyncInitial = async () => {
        ...
        let seqno = await this.client.getFeedSeq(this.id);
    ...
    #assumeHistoryEntities = async (items: { by: string, content: Content }[]) => {
        ...
        let { users } = this.#collectContentEntities(contents);
